...
const TestimonialsSection = () => {
  const { ref: headRef, shown: headShown } = useReveal({ threshold: 0.25 });
  return (
    <section className="container py-24" id="testimonials">
      <div ref={headRef as any} className={revealClass(headShown, 'mx-auto max-w-2xl text-center mb-14 space-y-4')}>
        <h2 className="text-3xl md:text-4xl font-bold playfair-display">Loved by organizers & attendees</h2>
          ...
          const { ref, shown } = useReveal({ threshold: 0.3 });
            return (
              <div
                ...
                className={revealClass(shown, 'relative rounded-2xl border bg-background/60 p-6 backdrop-blur group overflow-hidden')}
              >
